...
class MyGameManager : public QObject {
  ...
  Q_PROPERTY(QList<int> levelItems READ levelItems WRITE setlevelItems NOTIFY
                 levelItemsChanged FINAL)

 public:
  explicit MyGameManager(QObject* parent = nullptr);

  // property
  QList<int> levelItems() const { return item_levels_; }
 ...
 signals:
  ...
  // 下落动画信号
  void downRectAnimation(const QList<int>& needToMove, int x,
                         int y);  // 需要移动的块，被移走的块坐标
